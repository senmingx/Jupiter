...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "YHBTotefrSXeGpiO9qG41B76dWZebJut";
			...
			if (!embedded.isNull("venues")) {
				...
				if (venues.length() > 0) {
					...
					if (!venue.isNull("address")) {
						...
						if (!addr.isNull("line1")) {
							sb.append(addr.getString("line1"));
						}
						if (!addr.isNull("line2")) {
							sb.append(addr.getString("line2"));
						}
						if (!addr.isNull("line3")) {
							sb.append(addr.getString("line3"));
						}
						sb.append(", ");
					}
	...
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for (int i = 0; i < events.length(); i++) {
			...
			if (!event.isNull("rating")) {
				builder.setRating(event.getDouble("name"));
			}
